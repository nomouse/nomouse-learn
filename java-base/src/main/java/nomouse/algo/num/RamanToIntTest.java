 ...
 * <p>
 * I 可以放在 V (5) 和 X (10) 的左边，来表示 4 和 9。 X 可以放在 L (50) 和 C (100) 的左边，来表示 40 和 90。  C 可以放在 D (500) 和 M (1000) 的左边，来表示 400
 * 和 900。 给定一个罗马数字，将其转换成整数。输入确保在 1 到 3999 的范围内。
 * <p>
...
public class RamanToIntTest {
    ...
    public int romanToInt(String s) {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            int currVal = getValue(c);
            if (i + 1 < chars.length) {
                int nextVal = getValue(chars[i + 1]);
                if (nextVal > currVal) {
                    sum = sum - currVal;
                    continue;
                }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(new RamanToIntTest().romanToInt(r));
    }
